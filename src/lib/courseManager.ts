...
export const getCoursesWithStats = async (): Promise<CourseWithStats[]> => {
  if (!supabase) return [];

  // Get all courses with their lessons using a single query with join
  ...
  // Transform the data with stats
  return coursesData.map((c: any) => {
    const lessons = c.lessons || [];
    const totalDuration = lessons.reduce((acc: number, lesson: any) => {
